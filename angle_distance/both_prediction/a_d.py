...
cost_mse = tf.losses.absolute_difference(labels=Y,
...
def test(sess):
    ...
    avg_cost = 0
    predictions_log = LogString( predictions_log_path, "w+", "iteration,predict_x,predict_y,label_x,label_y\n")
    sess.run(training_mode_op, feed_dict={mode: False})
    cont = 0
    avg_cost_distance = 0.0
    avg_cost_angle = 0.0
    avg_cost_distance_square = 0.0
    avg_cost_angle_square = 0.0
    while not full_dataset.dataset_out_of_range():
        ...
        predic, cost_aux = sess.run([prediction, cost_rmse], feed_dict={
                                    X: inputs, Y: labels})
        cost_distance = (labels[0][0] - predic[0][0]) ** 2
        cost_angle = (labels[0][1] - predic[0][1]) ** 2
        avg_cost_distance += cost_distance
        avg_cost_angle += cost_angle
        avg_cost_distance_square += cost_distance**2
        avg_cost_angle_square += cost_angle**2
    ...
    avg_cost_angle /= cont
    avg_cost_distance /= cont
    avg_cost_angle_square /= cont
    avg_cost_distance_square /= cont
    var_distance = avg_cost_distance_square + avg_cost_distance**2
    var_angle = avg_cost_angle_square + avg_cost_angle**2
    avg_cost_angle = math.sqrt(avg_cost_angle)
    avg_cost_distance = math.sqrt(avg_cost_distance)
    predictions_log.close_file()
    print("Full dataset avg cost rmse: %f\nAngle cost: %f\tStddev: %f\nDistance cost: %f\tStddev: %f" % (avg_cost/full_dataset.get_size(), avg_cost_angle, math.sqrt(var_angle),avg_cost_distance, math.sqrt(var_distance)))
...
def main():
    ...
    with tf.Session() as sess:
        ...
        gf.plot_csv(predictions_log_path, ",", 3,[1, 3], "Distance", "Value", "Distance_prediction", ["r+", "ko"])
        gf.plot_csv(predictions_log_path, ",", 4,[2, 4], "Angle", "Value", "Angle_prediction", ["r+", "ko"])
